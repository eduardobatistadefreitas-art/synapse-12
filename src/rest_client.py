...
def requisitar_api_final(url_completa, headers, payload, timeout=15):
    """
    Mecanismo estável de tráfego. Simula um navegador real usando User-Agent 
    padrão de mercado para furar o bloqueio Cloudflare (HTML/Parse Error).
    """
    try:
        dados_bytes = json.dumps(payload).encode('utf-8')
        
        # Cabeçalhos cruciais para simulação de cliente legítimo
...
def orquestrar_chamada_rest(prompt_sistema, prompt_usuario):
    ...
    Rotas calibradas individualmente por provedor.
    """
    logs_erros = []

    # 1. NVIDIA Build (Fura o bloqueio HTML com User-Agent real)
    ...
    if status == 200:
        try:
            ...
            logs_erros.append(f"💥 NVIDIA Parse Erro: {str(e)} | Res: {res[:50]}")
    else:
        logs_erros.append(f"💥 NVIDIA Erro (Status {status}): {res[:100]}")

    # 2. OpenRouter (Tratamento contra estouro de cota e Cloudflare)
    ...
    if status == 200:
        try:
            ...
            logs_erros.append(f"编 OpenRouter Parse Erro: {str(e)} | Res: {res[:50]}")
    else:
        logs_erros.append(f"⚠️ OpenRouter Erro (Status {status}): {res[:100]}")

    # 3. Groq Cloud (Extingue o erro 405 ao remover cabeçalhos de Host manuais)
    ...
    if status == 200:
        try:
            ...
            logs_erros.append(f"💥 Groq Parse Erro: {str(e)} | Res: {res[:50]}")
    else:
        logs_erros.append(f"💥 Groq Erro (Status {status}): {res[:100]}")

    # 4. Gemini (Extingue o erro 400 usando o cabeçalho oficial x-goog-api-key)
    ...
    # Injeta a chave de autenticação no cabeçalho nativo exigido pelo Google
    headers_gemini = {"x-goog-api-key": key_gemini}
